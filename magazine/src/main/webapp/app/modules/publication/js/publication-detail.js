controllers.controller('PublicationDetailController', ['$scope', '$filter', '$stateParams', '$sce',
					   'PublicationFactory','PublicationsFactory', '$location','$localStorage',
    function ($scope, $filter, $stateParams,$sce, PublicationFactory, PublicationsFactory, $location, $localStorage) {
		pubObj.initialize($scope, $filter, $stateParams,$sce, PublicationFactory, PublicationsFactory, $location, $localStorage, true);
    }
]);

controllers.controller('PublicationCreateController', ['$scope', '$filter', '$stateParams', '$sce',
    'PublicationFactory', 'PublicationsFactory', '$location', '$localStorage',
    function ($scope, $filter,$stateParams, $sce, PublicationFactory,PublicationsFactory, $location, $localStorage) {
        pubObj.initialize($scope, $filter, $stateParams, $sce, PublicationFactory, PublicationsFactory, $location, $localStorage, false);
}]);

var pubObj = {
    initialize: function($scope, $filter, $stateParams, $sce, PublicationFactory, PublicationsFactory, $location, $localStorage,isUpdate){
        $scope.publication = {};
        ...
        $scope.insertOrUpdate = function () {
            ...
            $scope.publication.tblTeacherDto = {'teacherId':1, 'teacherCompany':1,'teacherContent':'',
